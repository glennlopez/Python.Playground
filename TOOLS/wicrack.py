...
class RunConfiguration:
    ...
    def __init__(self):
        ...
        # True means the program must find a valid handshake in order for wicrack to recognize a handshake.
        ...
        self.IFACE_TO_TAKE_DOWN = ''  # Interface that wicrack puts into monitor mode
    ...
    def ConfirmRunningAsRoot(self):
        if os.getuid() != 0:
            print R + ' [!]' + O + ' ERROR:' + G + ' wicrack' + O + ' must be run as ' + R + 'root' + W
            print R + ' [!]' + O + ' login as root (' + W + 'su root' + O + ') or try ' + W + 'sudo ./wicrack.py' + W
            exit(1)

    def ConfirmCorrectPlatform(self):
        if not os.uname()[0].startswith("Linux") and not 'Darwin' in os.uname()[0]:  # OSX support, 'cause why not?
            print O + ' [!]' + R + ' WARNING:' + G + ' wicrack' + W + ' must be run on ' + O + 'linux' + W
            exit(1)

    def CreateTempFolder(self):
        from tempfile import mkdtemp

        self.temp = mkdtemp(prefix='wicrack')
    ...
    def exit_gracefully(self, code=0):
        ...
        mac_change_back()
        print GR + " [+]" + W + " quitting"  # wicrack will now exit"
        print ''
        # GTFO
        exit(code)
    ...
    def build_opt_parser(self):
        ...
        global_group.add_argument('--update', help='Check and update wicrack.', default=False, action='store_true',
                                  dest='update')
    ...
    def upgrade(self):
        ...
        try:
            ...
            if revision == -1:
                ...
                try:
                    sock = urllib.urlopen('https://raw.githubusercontent.com/glennlopez/Python.Playground/master/TOOLS/wicrack.py')
                    page = sock.read()
                except IOError:
                    page = ''
                if page == '':
                ...
                f = open('update_wicrack.sh', 'w')
                f.write('''#!/bin/sh\n
                           rm -rf ''' + this_file + '''\n
                           mv wicrack_new.py ''' + this_file + '''\n
                           rm -rf update_wicrack.sh\n
                           chmod +x ''' + this_file + '''\n
                          ''')
                f.close()

                # Change permissions on the script
                returncode = call(['chmod', '+x', 'update_wicrack.sh'])
                if returncode != 0:
                    print R + ' [!]' + O + ' permission change returned unexpected code: ' + str(returncode) + W
                    self.exit_gracefully(1)
                # Run the script
                returncode = call(['sh', 'update_wicrack.sh'])
                if returncode != 0:
                ...
                print GR + ' [-]' + W + ' your copy of wicrack is ' + G + 'up to date' + W

        except KeyboardInterrupt:
            print R + '\n (^C)' + O + ' wicrack upgrade interrupted' + W
        self.exit_gracefully(0)
...
class RunEngine:
    ...
    def scan(self, channel=0, iface='', tried_rtl8187_fix=False):
        ...
        """
        remove_airodump_files(self.RUN_CONFIG.temp + 'wicrack')

        command = ['airodump-ng',
                   '-a',  # only show associated clients
                   '-w', self.RUN_CONFIG.temp + 'wicrack']  # output file
        if channel != 0:
            command.append('-c')
        ...
        try:
            ...
            while True:
                time.sleep(0.3)
                if not os.path.exists(self.RUN_CONFIG.temp + 'wicrack-01.csv') and time.time() - time_started > 1.0:
                    print R + '\n [!] ERROR!' + W
                    ...
                    print R + ' [!]' + O + ' wicrack is unable to generate airodump-ng output files' + W
                    print R + ' [!]' + O + ' you may want to disconnect/reconnect your wifi device' + W
                    self.RUN_CONFIG.exit_gracefully(1)

                (targets, clients) = self.parse_csv(self.RUN_CONFIG.temp + 'wicrack-01.csv')
                ...
                if self.RUN_CONFIG.VERBOSE_APS and len(targets) > 0:
                    ...
                    if not self.RUN_CONFIG.WPS_DISABLE:
                        wps_check_targets(targets, self.RUN_CONFIG.temp + 'wicrack-01.cap', verbose=False)
        ...
        if not self.RUN_CONFIG.WPS_DISABLE:
            wps_check_targets(targets, self.RUN_CONFIG.temp + 'wicrack-01.cap')
...
def banner(RUN_CONFIG):
    ...
    print W + "           ,-."
    print W + "        / \  `.  __..-,O"
    print W + "       :   \ --''_..-'.'"+BLD+"    Automated WiFi Auditor"
    ...
    print W + "       // || \\ "
    print W + "    __//__||__\\__"
    print R + "             __.___________  " +Y+ "private beta release"+R+"  __    "
    print R + "    /  \    /  \__\_   ___ \____________     ____ |  | __"
    print R + "    \   \/\/   /  /    \  \/\_  __ \__  \  _/ ___\|  |/ /"
    print R + "     \        /|  \     \____|  | \// __ \ \  \___|    <  v1.5 "
    print R + "      \__/\  / |__|\______  /|__|  (____  / \___  >__|_ \ "
    print R + "           \/             \/            \/      \/     \/ "
    print W
...
def help():
    ...
    print head + '\n   EXAMPLE' + W
    print sw + '\t./wicrack.py ' + W + '-wps -wep -c 6 -pps 600' + W
    print ''
